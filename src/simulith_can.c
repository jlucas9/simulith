...
static bool is_valid_config(const simulith_can_config_t *config) {
    ...
    if (config->bitrate < SIMULITH_CAN_BITRATE_125K || 
        config->bitrate > SIMULITH_CAN_BITRATE_1M) return false;
    
    // Validate sample point (50-90%)
    if (config->sample_point < 50 || config->sample_point > 90) return false;
    
    // Validate sync jump width (1-4)
    if (config->sync_jump < 1 || config->sync_jump > 4) return false;
...
static bool is_valid_message(const simulith_can_message_t *msg) {
    ...
    if (msg->is_extended) {
        if (msg->id > SIMULITH_CAN_ID_EXT_MAX) return false;
    } else {
        if (msg->id > SIMULITH_CAN_ID_STD_MAX) return false;
    }
    
    // Validate DLC
    if (msg->dlc > SIMULITH_CAN_MAX_DLC) return false;
    
    return true;
}

static bool message_passes_filter(const simulith_can_message_t *msg, const simulith_can_filter_t *filter) {
    if (msg->is_extended != filter->is_extended) return false;
...
int simulith_can_remove_filter(uint8_t bus_id, int filter_id) {
    if (bus_id >= MAX_CAN_BUSES || !can_buses[bus_id].initialized ||
        filter_id < 0 || filter_id >= MAX_FILTERS) {
        return -1;
    }
...
int simulith_can_send(uint8_t bus_id, const simulith_can_message_t *msg) {
    ...
    simulith_log("CAN%d TX: ID=0x%x [%d] %s%s",
                 bus_id, msg->id, msg->dlc,
    ...
    if (bus->rx_callback) {
        ...
        for (int i = 0; i < MAX_FILTERS; i++) {
            if (bus->filters[i].active &&
                message_passes_filter(msg, &bus->filters[i].filter)) {
                passes_filter = true;
                break;
            }
        }
...
int simulith_can_receive(uint8_t bus_id, simulith_can_message_t *msg) {
    ...
    simulith_log("CAN%d RX: ID=0x%x [%d] %s%s",
                 bus_id, msg->id, msg->dlc,
                 msg->is_extended ? "EXT " : "STD ",
                 msg->is_rtr ? "RTR " : "");
