...
static bool is_valid_pin_config(const simulith_gpio_config_t *config) {
    ...
    // Validate mode
    if (config->mode > SIMULITH_GPIO_MODE_OUTPUT_OD) return false;
    
    // Validate initial state for outputs
    if ((config->mode == SIMULITH_GPIO_MODE_OUTPUT || 
         config->mode == SIMULITH_GPIO_MODE_OUTPUT_OD) &&
        config->initial_state > 1) return false;
...
int simulith_gpio_write(uint8_t port, uint8_t pin, uint8_t value) {
    ...
    if (gpio_pin->mode != SIMULITH_GPIO_MODE_OUTPUT && 
        gpio_pin->mode != SIMULITH_GPIO_MODE_OUTPUT_OD) {
