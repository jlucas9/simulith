...
static bool is_valid_config(const simulith_spi_config_t *config) {
    ...
    if (config->clock_hz < 1000 || config->clock_hz > 100000000) return false;
    
    // Validate SPI mode (0-3)
    if (config->mode > SIMULITH_SPI_MODE_3) return false;
    
    // Validate bit order
    if (config->bit_order > SIMULITH_SPI_LSB_FIRST) return false;
    
    // Validate CS polarity
    if (config->cs_polarity > SIMULITH_SPI_CS_ACTIVE_HIGH) return false;
    
    // Validate data bits (4-16)
    if (config->data_bits < 4 || config->data_bits > MAX_DATA_BITS) return false;
...
int simulith_spi_init(uint8_t bus_id, const simulith_spi_config_t *config, 
    ...
    simulith_log("SPI bus %d initialized: %lu Hz, mode %d, %d bits %s first\n",
                 ...
                 config->mode,
                 config->data_bits,
                 config->bit_order == SIMULITH_SPI_MSB_FIRST ? "MSB" : "LSB");
    
    return 0;
}

int simulith_spi_transfer(uint8_t bus_id, uint8_t cs_id, 
